# Pre-Writing Reference Template

This document serves as a pre-writing reference for educational/informational content (articles defining concepts, explaining procedures, etc.). Use this template to establish clear audience understanding before writing.

**Note:** For general writing rules (structure, formatting, voice, banned phrases), see 'STYLE_GUIDE.md'. This document focuses on **audience-specific adjustments**.

---

## Part 1: About Us (Brief Introduction)

**Company Name**: Zhejiang Tianhong Transmission Machinery Co., Ltd. (Brand: TANHON)

**Industry**: Industrial Transmission Equipment Manufacturing,Non-automotive uses

**Core Business**: Research, development, manufacturing, and marketing of transmission equipment including gear reducers, gearboxes, gearmotors, and related power transmission components. The company specializes in mechanical power transmission solutions with 12 product lines covering power ranges from 0.18 to 4000 KW.

**Value Proposition**: TANHON delivers shorter delivery times, cost-effectiveness, more reasonable configurations, and enhanced stability in industrial transmission solutions. With approximately 40,000 transmission ratio configurations available, they provide comprehensive solutions for diverse industrial applications.

**Target Markets**:
- Geographic: South America, Eastern Europe, Southeast Asia, Africa, Middle East, Eastern Asia
- Industries: Manufacturing, industrial automation, material handling, food processing, mining, construction machinery

**Key Products/Services**:
1. Gear Reducers (MD, MN, MJ, MP, Z, W series)
2. Worm Gearboxes (NMRV series)
3. Helical Gear Units (MTH/MTB series)
4. Planetary Gearboxes
5. Gearmotors and speed reducers

**Factory Details**:
- Location: Linyang Industrial Park, Ruian, Wenzhou, Zhejiang, China
- 9 production lines
- Advanced testing facilities with precision grinding machines, CNC equipment, and gear measuring instruments
- Annual output: US$10-50 million

**Certifications**:
- ISO 9001 (Quality Management)
- ISO 14001 (Environmental Management)

**Sitemap URL** (for internal linking):
- Index: https://tanhon.com/sitemap_index.xml
- 子 sitemap 自动从 index 获取 (post-sitemap, page-sitemap, product-sitemap 等)
- 非英语链接自动过滤 (/fr/, /it/, /de/ 等路径)

---

## Part 2: Target User Analysis

### User Type 1: The Complete Beginner

| Attribute | Description |
|-----------|-------------|
| **Industry** | New plant managers, maintenance trainees, engineering students, small business owners exploring automation |
| **Knowledge Level** | Novice (0-6 months exposure) |
| **Goals** | Understand what gearboxes do; learn basic terminology; make informed initial equipment decisions |

**What They Already Know**:
- Basic awareness that machines need power transmission
- May have heard terms like "gearbox" or "reducer" without understanding differences
- General mechanical common sense only

**What They Need to Learn**:
- What a gearbox/gear reducer does and why it's necessary
- Basic types of gearboxes and their differences
- Key specifications to consider (ratio, power, torque)
- When to use which type of transmission equipment

**Questions They Search For**:
- "What is a gearbox?"
- "Gear reducer explained"
- "How do gearmotors work?"
- "Types of industrial gearboxes"
- "Gearbox vs gear reducer difference"
- "Why use a speed reducer?"

**Typical Roles/Titles**:
- New plant managers evaluating equipment
- Maintenance trainees learning basic machinery
- Small business owners exploring automation options
- Engineering students in mechanical/industrial programs

**Pain Points & Challenges**:
- Overwhelmed by technical specifications and jargon
- Uncertainty about which gearbox type suits their needs
- Fear of making costly purchasing mistakes
- Difficulty communicating requirements to suppliers

**Content Preferences**:

| Aspect | Preference |
|--------|------------|
| **Reading Time** | 5-8 minutes |
| **Device** | Mobile & desktop (split evenly) |
| **Content Length** | 1,200-1,800 words |
| **Format** | Visual diagrams, short paragraphs, numbered lists |

**Decision Factors**:
- Clear explanations without overwhelming detail
- Real-world examples they can relate to
- Confidence that they understand the basics
- Knowing where to go for more advanced information

**Writing Approach**:

| Element | Recommendation |
|---------|----------------|
| **Tone** | Friendly, encouraging, patient |
| **Style** | Conversational; use analogies and real-world examples |
| **Complexity** | Simple sentences; define all jargon; avoid acronyms or explain them immediately |
| **Structure** | Start with "why it matters"; build concepts step-by-step |
| **Word Choice** | Everyday language; assume no prior knowledge |

**Audience-Specific Guidelines**:

✓ Use everyday analogies: "Like the gears on a bicycle..."
✓ Anticipate confusion: "This is where many people get confused, so let's slow down."
✓ Provide context before details: "Before we dive into types, let's understand why this matters."
✓ Use transitional phrases: "Here's the good news..." / "The key takeaway is..."

✗ Never assume prior knowledge: Don't write "As you probably know..."
✗ Avoid unexplained acronyms: Never write "Check the RPM" without defining it first
✗ Don't chain multiple concepts: Don't introduce ratio, torque, and efficiency in one paragraph
✗ Avoid being condescending: Don't write "This is easy" or "Simply do X"

---

### User Type 2: The Informed Non-Expert

| Attribute | Description |
|-----------|-------------|
| **Industry** | Procurement managers, plant supervisors, project engineers in adjacent fields, maintenance coordinators |
| **Knowledge Level** | Basic to Intermediate (6 months - 2 years exposure) |
| **Goals** | Select the right gearbox for applications; communicate effectively with suppliers; evaluate quotes and specifications |

**What They Already Know**:
- General concepts of power transmission
- Basic gearbox terminology and common types
- Awareness of major brands and suppliers
- Some hands-on experience with equipment selection

**What They Need to Learn**:
- Selection criteria and comparison frameworks
- How to match gearbox specifications to application requirements
- Common pitfalls in gearbox selection and installation
- Best practices for procurement and maintenance planning

**Questions They Search For**:
- "How to choose a gearbox"
- "Helical vs worm gearbox comparison"
- "Gearbox sizing guide"
- "Best gearbox for conveyor application"
- "Gear reducer selection checklist"
- "Common gearbox problems"

**Typical Roles/Titles**:
- Procurement managers sourcing transmission equipment
- Plant supervisors overseeing equipment selection
- Project engineers coordinating installations
- Maintenance coordinators planning replacements

**Pain Points & Challenges**:
- Balancing cost vs. performance in equipment selection
- Evaluating conflicting vendor recommendations
- Ensuring compatibility with existing systems
- Managing downtime during equipment replacement

**Content Preferences**:

| Aspect | Preference |
|--------|------------|
| **Reading Time** | 6-10 minutes |
| **Device** | Desktop (primary), tablet |
| **Content Length** | 1,500-2,500 words |
| **Format** | Comparison tables, checklists, decision trees |

**Decision Factors**:
- Clear selection criteria and decision frameworks
- Practical comparisons between options
- Vendor-neutral guidance they can trust
- Actionable next steps for procurement

**Writing Approach**:

| Element | Recommendation |
|---------|----------------|
| **Tone** | Professional, practical, direct |
| **Style** | Action-oriented; focus on decision-making frameworks |
| **Complexity** | Moderate; can use industry terms with brief clarification |
| **Structure** | Problem-solution format; comparison tables; numbered lists |
| **Word Choice** | Industry-standard terms; occasional technical vocabulary |

**Audience-Specific Guidelines**:

✓ Provide decision frameworks: "Ask yourself these three questions..."
✓ Use comparison tables for gearbox types
✓ Offer concrete criteria: "Look for service factors of at least 1.5 for heavy-duty applications"
✓ Acknowledge trade-offs: "Higher efficiency often means higher upfront cost."

✗ Don't over-explain basics they already know
✗ Avoid deep technical details without practical relevance
✗ Don't be wishy-washy: Avoid "It depends" without explaining what it depends on
✗ Don't overwhelm with options: Curate and recommend, don't just list

---

### User Type 3: The Practitioner / Implementer

| Attribute | Description |
|-----------|-------------|
| **Industry** | Maintenance engineers, mechanical technicians, installation specialists, field service engineers |
| **Knowledge Level** | Intermediate to Advanced (2-5 years experience) |
| **Goals** | Install gearboxes correctly; troubleshoot problems; perform maintenance; optimize performance |

**What They Already Know**:
- Solid understanding of gearbox fundamentals
- Hands-on experience with installation and maintenance
- Familiarity with common failure modes
- Technical vocabulary and industry standards

**What They Need to Learn**:
- Advanced troubleshooting techniques
- Proper alignment and installation procedures
- Lubrication specifications and maintenance schedules
- New product features and updates

**Questions They Search For**:
- "How to align gearbox to motor"
- "Gearbox oil change procedure"
- "Worm gearbox troubleshooting"
- "Gearbox vibration causes"
- "Gear reducer installation manual"
- "Bearing replacement gearbox"

**Typical Roles/Titles**:
- Maintenance engineers responsible for equipment reliability
- Mechanical technicians performing installations
- Field service engineers troubleshooting issues
- Installation specialists on industrial projects

**Pain Points & Challenges**:
- Diagnosing complex vibration and noise problems
- Ensuring proper alignment within tight tolerances
- Managing lubrication schedules across multiple units
- Accessing technical specifications quickly on-site

**Content Preferences**:

| Aspect | Preference |
|--------|------------|
| **Reading Time** | 8-12 minutes |
| **Device** | Mobile (field use), desktop (planning) |
| **Content Length** | 2,000-3,000 words |
| **Format** | Step-by-step procedures, technical diagrams, specification tables |

**Decision Factors**:
- Accuracy of technical specifications
- Clarity of procedural steps
- Troubleshooting decision trees
- Downloadable reference materials

**Writing Approach**:

| Element | Recommendation |
|---------|----------------|
| **Tone** | Technical, precise, matter-of-fact |
| **Style** | Procedural; detailed step-by-step instructions |
| **Complexity** | High; assume technical competence; include specifications |
| **Structure** | Sequential steps; specifications; technical diagrams |
| **Word Choice** | Full technical vocabulary; use industry shorthand freely |

**Audience-Specific Guidelines**:

✓ Use numbered lists for sequential steps
✓ Bold key specifications and warnings
✓ Include units with all numerical values
✓ Reference applicable standards (ISO, AGMA, etc.)

✗ Don't over-explain fundamental concepts
✗ Avoid vague instructions: "tighten securely" → "torque to 45 Nm"
✗ Don't skip safety warnings or prerequisites
✗ Avoid marketing language or promotional content

---

### User Type 4: The Strategic Decision-Maker

| Attribute | Description |
|-----------|-------------|
| **Industry** | Plant directors, operations executives, capital equipment buyers, engineering managers |
| **Knowledge Level** | Conceptually advanced; operationally variable |
| **Goals** | Justify equipment investments; optimize total cost of ownership; manage reliability and risk |

**What They Already Know**:
- Business context and operational requirements
- High-level understanding of transmission equipment
- Financial and operational implications
- Vendor landscape

**What They Need to Learn**:
- ROI calculations for equipment upgrades
- Total cost of ownership analysis
- Reliability improvements and risk mitigation
- Industry trends and technology developments

**Questions They Search For**:
- "Gearbox ROI calculator"
- "Industrial gearbox market trends"
- "Equipment reliability improvement strategies"
- "Gearbox vs direct drive comparison"
- "Transmission equipment lifecycle cost"
- "Premium vs standard gearbox value"

**Typical Roles/Titles**:
- Plant directors overseeing capital investments
- Operations executives managing equipment strategy
- Engineering managers leading reliability programs
- Capital equipment buyers evaluating major purchases

**Pain Points & Challenges**:
- Justifying capital expenditures to senior leadership
- Balancing upfront costs with total cost of ownership
- Managing equipment reliability across multiple facilities
- Evaluating vendor claims and market positioning

**Content Preferences**:

| Aspect | Preference |
|--------|------------|
| **Reading Time** | 5-7 minutes |
| **Device** | Desktop, tablet |
| **Content Length** | 1,500-2,200 words |
| **Format** | Executive summaries, data charts, ROI calculators, case studies |

**Decision Factors**:
- Financial impact and ROI metrics
- Risk mitigation and reliability data
- Industry benchmarks and competitive analysis
- Strategic alignment with operational goals

**Writing Approach**:

| Element | Recommendation |
|---------|----------------|
| **Tone** | Authoritative, strategic, confident |
| **Style** | Executive summary format; data-driven; outcome-focused |
| **Complexity** | Conceptually sophisticated but technically accessible |
| **Structure** | Lead with conclusions; support with evidence; use metrics |
| **Word Choice** | Business vocabulary; industry context; avoid deep technical details |

**Audience-Specific Guidelines**:

✓ Lead with business impact and ROI
✓ Use data and metrics to support claims
✓ Frame technical details in business outcomes
✓ Include industry benchmarks and comparative analysis

✗ Don't dive into technical minutiae without context
✗ Avoid technical jargon without business translation
✗ Don't make claims without supporting evidence
✗ Avoid one-size-fits-all recommendations

---

### User Type 5: The Expert / Specialist

| Attribute | Description |
|-----------|-------------|
| **Industry** | Gearbox design engineers, transmission specialists, reliability consultants, R&D professionals |
| **Knowledge Level** | Expert (5+ years; deep specialization) |
| **Goals** | Stay current on technology; discover nuances; validate approaches; find authoritative references |

**What They Already Know**:
- Comprehensive understanding of gear design and transmission theory
- Historical context and evolution of gearbox technology
- Multiple design approaches and their trade-offs
- Current limitations and emerging solutions

**What They Need to Learn**:
- Latest developments in materials and manufacturing
- Nuanced analysis of edge cases
- Alternative design perspectives
- Advanced applications and innovations

**Questions They Search For**:
- "Helical gear mesh efficiency optimization"
- "Gearbox thermal management advances"
- "ISO 6336 gear rating updates"
- "Planetary gearbox torque density comparison"
- "High-speed gearbox design considerations"
- "Gearbox noise reduction techniques"

**Typical Roles/Titles**:
- Gearbox design engineers developing new products
- Transmission specialists solving complex problems
- Reliability consultants optimizing performance
- R&D professionals researching innovations

**Pain Points & Challenges**:
- Staying current with evolving standards and materials
- Optimizing designs for conflicting requirements
- Validating theoretical approaches with field data
- Accessing authoritative technical references quickly

**Content Preferences**:

| Aspect | Preference |
|--------|------------|
| **Reading Time** | 10-15 minutes |
| **Device** | Desktop (primary) |
| **Content Length** | 2,500-4,000 words |
| **Format** | Technical papers, detailed analysis, equations, CAD examples |

**Decision Factors**:
- Technical depth and accuracy
- Citation of standards and research
- Novel insights or perspectives
- Applicability to edge cases and complex scenarios

**Writing Approach**:

| Element | Recommendation |
|---------|----------------|
| **Tone** | Peer-to-peer, analytical, nuanced |
| **Style** | Thorough analysis; acknowledge complexity and trade-offs |
| **Complexity** | Highest; assume full domain knowledge |
| **Structure** | Thesis-driven; comprehensive coverage; cite sources |
| **Word Choice** | Expert vocabulary; reference standards and research |

**Audience-Specific Guidelines**:

✓ Reference current standards (ISO 6336, AGMA 2001, DIN 3990)
✓ Acknowledge complexity and edge cases
✓ Cite research and provide technical depth
✓ Include mathematical derivations where relevant

✗ Don't oversimplify complex phenomena
✗ Avoid unsupported generalizations
✗ Don't ignore alternative approaches or trade-offs
✗ Avoid promotional content masquerading as technical analysis

---

## Part 3: Quick Reference Summary

| User Type | Knowledge Level | Primary Goal | Tone | Complexity |
|-----------|-----------------|--------------|------|------------|
| Complete Beginner | Novice | Understand basics | Friendly, patient | Simple |
| Informed Non-Expert | Basic-Intermediate | Select equipment | Professional, practical | Moderate |
| Practitioner | Intermediate-Advanced | Install & maintain | Technical, precise | High |
| Decision-Maker | Conceptually advanced | Justify investments | Authoritative, strategic | Conceptually high |
| Expert | Expert | Stay current | Peer-to-peer, analytical | Highest |

---

## Part 4: Products & Services Context

**Purpose:** Enable natural product mentions in educational content by understanding what problems each product solves and when to mention it.

### Product Categories

#### Category: Helical Gear Reducers

| Attribute | Details |
|-----------|---------|
| **Products** | MTH/MTB series helical gear units, inline helical reducers, parallel shaft reducers |
| **Problems Solved** | Efficient speed reduction; high torque transmission; quiet operation; precise speed control |
| **Technical Advantages** | High efficiency (95%+); low noise; compact design; multiple mounting options; long service life |
| **Applicable Standards** | ISO 6336, DIN 3990, AGMA 2001 |
| **Typical Applications** | Conveyors, mixers, agitators, packaging equipment, material handling, general industrial machinery |

**Topic Relevance:**
- "helical gearbox" → High relevance
- "gear reducer selection" → High relevance
- "industrial speed reduction" → High relevance
- "conveyor drives" → Medium relevance

**Natural Mention Triggers:**

| When Article Discusses | Natural Mention Opportunity |
|------------------------|----------------------------|
| High-efficiency applications | "Helical gear reducers achieve efficiencies above 95% per stage" |
| Noise-sensitive environments | "The gradual tooth engagement of helical gears produces quieter operation than spur gears" |
| Conveyor systems | "Inline helical reducers provide efficient, compact drives for conveyor applications" |

---

#### Category: Worm Gearboxes

| Attribute | Details |
|-----------|---------|
| **Products** | NMRV series worm reducers, worm gearmotors, right-angle worm drives |
| **Problems Solved** | Compact right-angle drive; high reduction ratios in single stage; self-locking capability |
| **Technical Advantages** | Space-efficient right-angle design; single-stage ratios up to 100:1; self-locking option; quiet operation |
| **Applicable Standards** | ISO 14521, DIN 3975, AGMA 6034 |
| **Typical Applications** | Packaging machinery, conveyors with holding requirements, gate operators, light-duty positioning |

**Topic Relevance:**
- "worm gearbox" → High relevance
- "right-angle drive" → High relevance
- "self-locking gearbox" → High relevance
- "NMRV reducer" → High relevance

**Natural Mention Triggers:**

| When Article Discusses | Natural Mention Opportunity |
|------------------------|----------------------------|
| Space constraints | "Worm gearboxes provide compact right-angle drives where space is limited" |
| High reduction ratios | "Single-stage worm reducers achieve ratios up to 100:1" |
| Holding loads | "The self-locking characteristic of worm drives prevents backdriving under load" |

---

#### Category: Planetary Gearboxes

| Attribute | Details |
|-----------|---------|
| **Products** | Planetary gear reducers, inline planetary gearheads, precision planetary units |
| **Problems Solved** | High torque density; compact high-ratio reduction; precision motion control |
| **Technical Advantages** | Highest torque density; coaxial input/output; multiple stages for high ratios; handles shock loads |
| **Applicable Standards** | ISO 6336, DIN 3990 |
| **Typical Applications** | Robotics, servo systems, heavy-duty drives, hoisting equipment, precision positioning |

**Topic Relevance:**
- "planetary gearbox" → High relevance
- "high torque density" → High relevance
- "servo gearhead" → Medium relevance
- "precision reducer" → Medium relevance

**Natural Mention Triggers:**

| When Article Discusses | Natural Mention Opportunity |
|------------------------|----------------------------|
| High torque requirements | "Planetary gearboxes offer the highest torque density for a given size" |
| Compact drive solutions | "The load-sharing of planetary gear sets enables compact, high-capacity designs" |
| Precision applications | "Precision planetary reducers minimize backlash for accurate positioning" |

---

#### Category: Gearmotors

| Attribute | Details |
|-----------|---------|
| **Products** | Helical gearmotors, worm gearmotors, integrated motor-reducer units |
| **Problems Solved** | Complete drive package; simplified installation; matched motor-reducer performance |
| **Technical Advantages** | Pre-engineered combinations; reduced installation time; optimized motor-reducer matching; compact footprint |
| **Applicable Standards** | IEC motor standards, reducer standards |
| **Typical Applications** | Conveyors, mixers, fans, pumps, material handling, food processing equipment |

**Topic Relevance:**
- "gearmotor selection" → High relevance
- "integrated drive" → High relevance
- "motor reducer combination" → Medium relevance

**Natural Mention Triggers:**

| When Article Discusses | Natural Mention Opportunity |
|------------------------|----------------------------|
| Drive system selection | "Gearmotors provide pre-matched motor and reducer combinations for simplified specification" |
| Installation considerations | "Integrated gearmotors reduce installation time compared to separate motor and reducer mounting" |
| Compact drive needs | "Gearmotor packages offer space savings over separate component arrangements" |

---

### Product Mention Guidelines

| Guideline | Rule |
|-----------|------|
| **Max Mentions** | 1-2 product references per article (solution-focused) |
| **Style** | Educational, not promotional. Product is THE SOLUTION, not the hero |
| **Placement** | Within relevant technical discussion, not intro/conclusion |
| **Tone** | "This can be addressed by..." NOT "Our product is..." |
| **Linking** | Link to product page only if genuinely helpful for reader |

**What Makes a Good Product Mention:**
- ✅ Appears where reader is looking for a solution
- ✅ Phrased as industry solution, not company advertisement
- ✅ Provides technical value (specs, capabilities)
- ✅ Reader would find it helpful even without buying

**What Makes a Bad Product Mention:**
- ❌ Feels inserted/forced into unrelated content
- ❌ Uses promotional language ("best", "leading", "superior")
- ❌ Appears in intro or conclusion (too promotional)
- ❌ Multiple mentions of same product

---

### Product-Topic Matching Reference

| Topic Keywords | Relevant Product Category | Mention Priority |
|----------------|---------------------------|------------------|
| helical gear, helical reducer, MTH, MTB | Helical Gear Reducers | High |
| gear efficiency, low noise gearbox | Helical Gear Reducers | High |
| worm gear, NMRV, right-angle drive | Worm Gearboxes | High |
| self-locking, backdrive prevention | Worm Gearboxes | High |
| planetary gear, high torque | Planetary Gearboxes | High |
| servo gearhead, precision motion | Planetary Gearboxes | High |
| gearmotor, integrated drive | Gearmotors | High |
| conveyor drive, mixer drive | Helical Gear Reducers, Gearmotors | Medium |
| speed reducer selection | Helical, Worm, Planetary | Medium |
| gear ratio, speed reduction | All categories | Medium |

---

## How to Use This Template

1. **Reference Part 1** to understand TANHON's business context and product focus
2. **Identify target audience** from Part 2 based on article topic and intent
3. **Apply appropriate tone and style guidelines** for the selected audience
4. **Cross-reference Part 3** for quick guidance during writing
5. **Reference Part 4** for natural product mention opportunities
6. **Use with STYLE_GUIDE.md** for complete writing guidelines

---

## Summary

工业传动设备制造商，产品涵盖斜齿轮减速机(MTH/MTB系列)、蜗轮蜗杆减速机(NMRV系列)、行星减速机、齿轮电机等12个产品线，功率范围0.18-4000KW，约40000种传动比配置可选，服务南美、东欧、东南亚、非洲、中东等市场，制造基地位于浙江瑞安

---

## Part 5: Author Personas

**Purpose:** Define consistent "voices" for content. Writers select a persona at the start of each article.

### Persona 1: 技术专家 / Technical Expert

| Attribute | Description |
|-----------|-------------|
| **Best For** | Gear technology, reducer selection, technical specifications |
| **Role** | Senior Drive Systems Engineer |
| **Experience** | 15+ years in power transmission design |
| **Specialty** | Gear geometry, reducer sizing, efficiency optimization |
| **Bias** | Service factor is not optional padding; gear quality grade determines lifespan; proper lubrication prevents 60% of failures |
| **Voice Traits** | Calculation-focused, references AGMA/ISO standards, precision-oriented |

**Signature Phrases:**
- "The torque calculation must include..."
- "Per AGMA standards, the service factor..."
- "Gear accuracy class determines..."
- "The efficiency loss at each stage is..."

---

### Persona 2: 实践导师 / Practical Mentor

| Attribute | Description |
|-----------|-------------|
| **Best For** | Installation guides, maintenance procedures, troubleshooting |
| **Role** | Field Service Engineer / Maintenance Trainer |
| **Experience** | 10+ years in industrial drive maintenance |
| **Specialty** | Reducer installation, alignment, oil analysis |
| **Bias** | Proper alignment prevents premature bearing failure; oil change intervals are minimums, not targets; vibration monitoring catches problems early |
| **Voice Traits** | Hands-on, maintenance-focused, practical tips |

**Signature Phrases:**
- "Before mounting the reducer, check..."
- "The alignment tolerance should be..."
- "Oil analysis will show..."
- "The vibration pattern indicates..."

---

### Persona 3: 行业观察者 / Industry Analyst

| Attribute | Description |
|-----------|-------------|
| **Best For** | Technology trends, efficiency comparisons, selection guides |
| **Role** | Power Transmission Consultant |
| **Experience** | Cross-industry experience in drive systems |
| **Specialty** | Reducer technology trends, energy efficiency, lifecycle cost |
| **Bias** | Energy efficiency savings often exceed purchase price over lifecycle; IE4/IE5 motors change reducer requirements; predictive maintenance is becoming standard |
| **Voice Traits** | Efficiency-focused, lifecycle cost perspective, trend-aware |

**Signature Phrases:**
- "The industry trend toward..."
- "When comparing lifecycle costs..."
- "Energy efficiency regulations now..."
- "The selection criteria have shifted..."

---

### Persona Selection Guide

| Article Type | Recommended Persona |
|--------------|---------------------|
| Reducer sizing / Selection | Persona 1: 技术专家 |
| Installation / Maintenance | Persona 2: 实践导师 |
| Technology trends / Comparisons | Persona 3: 行业观察者 |
| Troubleshooting / Failure analysis | Persona 2: 实践导师 |
| Efficiency optimization | Persona 1 or 3 |
